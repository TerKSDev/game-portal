...
export default async function Home(props: {
  ...
  // If user has searched, show search results with View More layout
  if (query) {
    const searchResults = await GetGames(1, gamesLimit, query, filters);
        ...
        <div className="flex flex-col w-full max-w-7xl gap-6 sm:gap-8 pb-8">
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4">
              <div>
                <h1 className="text-3xl sm:text-5xl font-black bg-linear-to-r from-cyan-400 to-blue-400 bg-clip-text text-transparent">
                  Search Results
                </h1>
              ...
              <div className="flex gap-2">
                ...
                <div className="flex gap-1 bg-zinc-900/50 border border-zinc-700/50 rounded-lg p-1">
                  <Link
                    href={`/?query=${query}&loadMore=${loadMore}&viewType=grid`}
                  ...
                  <Link
                    href={`/?query=${query}&loadMore=${loadMore}&viewType=list`}
                    className={`p-2 rounded transition-all ${
                      viewType === "list"
                        ? "bg-zinc-700 text-white"
                        : "text-zinc-400 hover:text-zinc-300"
          ...
          {searchResults.length === 0 && (
            <div className="flex flex-col items-center justify-center py-16">
              <IoGameController className="text-zinc-600" size={64} />
              <h3 className="text-xl font-bold text-zinc-400 mt-4">
                No results found
              </h3>
              <p className="text-zinc-500 text-sm mt-2">
                {query
                  ? "Try adjusting your search or browse our categories"
                  : "Unable to load games at this time. Please try again later."}
              </p>
              {!query && (
                <Link
                  href="/"
          ...
          {/* Grid View */}
          {searchResults.length > 0 && viewType === "grid" && (
          ...
          {/* List View */}
          {searchResults.length > 0 && viewType === "list" && (
      ...
      return (
        <main className="flex flex-1 w-full min-h-screen justify-center px-4 sm:px-6 lg:px-8 py-8">
          <div className="flex flex-col w-full max-w-7xl gap-6 sm:gap-8 pb-8">
            ...
            {config.games.length > 0 && viewType === "grid" && (
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                {config.games.map(
                  (game: {
                    id: number;
            ...
            {config.games.length > 0 && viewType === "list" && (
              <div className="flex flex-col gap-3">
                {config.games.map(
                  (game: {
                    id: number;
  ...
  // Homepage: fetch all data in parallel (only when not in search or view mode)
  // Store Highlights only shows games with Steam links
  const homepageFilters = { ...filters, stores: "1" };

  const [games, newReleases, highestRated, bestMetacritic, session] =
    await Promise.all([
      GetGames(1, 24, "", homepageFilters), // Only Steam games
      GetGames(1, 5, "", { ordering: "-released", stores: "1" }),
      GetGames(1, 5, "", { ordering: "-rating", stores: "1" }),
      GetGames(1, 5, "", { ordering: "-metacritic", stores: "1" }),
      auth(),
    ]);
  ...
  if (session?.user?.id) {
    try {
      ...
      console.error("[Homepage] Library count query error:", error);
          ...
          <div className="bg-amber-900/20 border border-amber-700/50 rounded-xl p-6 flex items-start gap-4">
            ...
            <div>
              ...
              <p className="text-zinc-300 text-sm mb-2">
                We're experiencing temporary issues connecting to our game
                database. This might be due to:
              </p>
              <ul className="text-zinc-400 text-sm list-disc list-inside space-y-1 mb-3">
                <li>High traffic on the game API</li>
