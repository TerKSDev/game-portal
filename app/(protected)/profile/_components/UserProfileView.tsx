      ...
      <div className="max-w-7xl mx-auto w-full">
        ...
        <div className="flex flex-row flex-wrap gap-6 max-lg:flex-col">
          <div className="flex flex-2 flex-col gap-6">
            {/* User Profile Card */}
            <div className="flex flex-col sm:flex-row bg-zinc-900/50 backdrop-blur-md border border-zinc-800/80 p-6 rounded-2xl shadow-2xl h-fit gap-6">
              <div className="relative mx-auto sm:mx-0">
                <div className="w-32 h-32 sm:w-40 sm:h-40 rounded-2xl overflow-hidden border-2 border-zinc-700/80 shadow-lg">
                  {userData.image ? (
                    <Image
                      src={userData.image}
                      alt="Avatar"
              ...
              <div className="flex flex-col sm:flex-row w-full gap-4 sm:gap-x-6 justify-between">
                <div className="flex flex-col gap-y-4 justify-between h-full flex-1">
                  <div className="flex flex-col">
                    <h2 className="w-full text-xl sm:text-2xl font-bold text-zinc-200 px-3 pb-0">
                ...
                <div className="flex flex-col sm:justify-between gap-4 items-start sm:items-end h-full w-full sm:w-auto">
                  <div className="w-full sm:w-auto">
                    <div className="flex flex-row items-center gap-x-2 rounded-full bg-zinc-800/50 border border-zinc-700/50 px-4 py-2 w-full sm:w-auto justify-between sm:justify-start">
                      {getStatusDisplay(userData.userStatus)}
                    </div>
                  </div>

                  {!isFriend && (
                    <div className="text-sm text-zinc-400 italic px-4 py-2">
            ...
            {mutualFriends.length > 0 && (
              <div className="flex flex-col bg-zinc-900/50 backdrop-blur-md border border-zinc-800/80 p-5 rounded-2xl shadow-2xl gap-4">
                  ...
                  {mutualFriends.map((friend) => (
                    <div
                      ...
                      className={`flex flex-col items-center gap-2 bg-zinc-800/50 rounded-xl p-3 border border-zinc-700/50 transition-all duration-300 ${
                        friend.uid
                          ? "hover:border-blue-500/50 hover:bg-zinc-700/50 cursor-pointer"
                          : "opacity-70"
                      }`}
                    >
          ...
          <div className="flex flex-col gap-6 flex-1">
                ...
                <div className="flex flex-col gap-3">
                  {libraryData.slice(0, 3).map((item) => (
                    <Link
                      ...
                      key={item.id}
                      className="flex flex-row items-center gap-3 bg-zinc-800/50 rounded-xl p-3 border border-zinc-700/50 hover:border-zinc-600/50 transition-colors"
                    >
                      <div className="relative w-14 h-14 rounded-xl overflow-hidden bg-zinc-700">
                        <Image
                          src={item.image}
                          alt={item.name}
                          fill
            ...
            <button
              ...
              className="flex items-center justify-center gap-2 bg-linear-to-r from-blue-600 to-blue-500 hover:from-blue-500 hover:to-blue-400 text-white px-6 py-3 rounded-xl font-bold transition-all duration-300 shadow-lg hover:shadow-blue-500/50"
            >
              <FaUserFriends />
              Back to Friends
            </button>
